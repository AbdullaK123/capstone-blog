...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            flash("You've already signed up with that email. Please use another one.")
            return redirect(url_for('register',form=form,logged_in=logged_in()))
        else:
            db.session.commit()
            login_user(new_user)
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not logged_in():
            ...
            return redirect(url_for('login', form=LoginForm(), logged_in=logged_in()))
        comment = Comment(text=comment_form.text.data, author_id=current_user.id, post_id=post_id)
        db.session.add(comment)
        db.session.commit()
        return render_template('post.html',post=requested_post, user=current_user,comment_form=comment_form,logged_in=logged_in(), comments=Comment.query.filter_by(post_id=post_id).all(),gravatar=gravatar)
    return render_template("post.html", post=requested_post, user=current_user, comment_form=comment_form,logged_in=logged_in(),comments=Comment.query.filter_by(post_id=post_id).all(),gravatar=gravatar)
